  ...
  >
    ...
    >
      ...
      >
        ...
        <span
          ...
          >ثبّت خطوط الباركود أفقياً بالمنتصف</span
        >
      </div>
    </div>
    ...
    >
      <p class="text-xs font-mono font-bold text-slate-400">
        <span class="text-slate-500 font-sans">جاري التحليل:</span>
        <span class="text-emerald-400 ml-1 text-sm tracking-widest">{{
          currentDecodingAttempt || 'في انتظار محاذاة الباركود...'
        }}</span>
      </p>
    </div>
    ...
    <div class="w-full flex items-center justify-between mt-3 px-1">
      <div class="flex items-center gap-2">
        ...
        <span class="text-xs font-bold text-slate-400">مصفّي القراءات العشوائية نشط</span>
      </div>
...
const initQuagga = () => {
  Quagga.init(
    ...
    (err) => {
      if (err) {
        emit('error', 'فشل تشغيل محرك الكاميرا: ' + err.message)
        emit('close')
        return
      }
      Quagga.start()

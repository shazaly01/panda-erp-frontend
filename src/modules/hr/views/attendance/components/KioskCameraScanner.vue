        ...
        >
          ...
          {{
            isEmergencyMode
              ...
              : 'تفعيل وضع الطوارئ (كاميرا الهاتف / إدخال يدوي)'
          }}
        </button>
      </div>
...
// الدالة المركزية الموحدة لاستقبال وتجهيز الأكواد
const processAttendanceCode = async (code) => {
  if (!code || attendanceStore.loading) return

  // 🔥 التحديث الجديد: طباعة الكود المقروء فوراً في خانة الأرقام اليدوية ليظهر أمام فرد الأمن
  if (isEmergencyMode.value) {
  ...
  try {
    ...
    // محاكاة تأخير طفيف جداً لمنح الـ UI فرصة لإعادة بناء التحولات الحركية بسلاسة
    setTimeout(() => {
      scanResult.value = {
        status: data.status,
        action: data.action,
    ...
    // تصفير خانة العرض بعد الانتهاء التام لتهيئتها للموظف التالي
    manualEmployeeNumber.value = ''

<!--src\modules\hr\views\positions\components\PositionsTable.vue-->
<template>
  <AppCard class="overflow-hidden">
    <AppTable :headers="tableHeaders" :items="positions" :is-loading="loading">
      <template #cell-position_info="{ item }">
      ...
      <template #cell-description="{ item }">
        <div class="text-xs text-text-muted max-w-xs truncate" :title="item.description">
          {{ item.description || '---' }}
        </div>
      </template>
      ...
      <template #cell-actions="{ item }">
        <div class="flex items-center justify-end space-x-1 space-x-reverse">
          <button
            v-if="authStore.can('position.update')"
          ...
          <button
            v-if="authStore.can('position.delete')"
            ...
            :title="item.employees_count > 0 ? 'لا يمكن الحذف لارتباطه بموظفين' : 'حذف'"
...
const tableHeaders = computed(() => [
  { key: 'position_info', label: 'المسمى الوظيفي', class: 'min-w-[200px]' },
  { key: 'description', label: 'الوصف', class: 'min-w-[200px]' },
  { key: 'employees_count', label: 'المرتبطين', class: 'min-w-[120px]' },

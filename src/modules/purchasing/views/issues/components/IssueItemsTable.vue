...
<template>
  <div class="p-0 bg-transparent space-y-3 font-sans text-right" dir="rtl">
    ...
    <!-- جدول إدخال وتعديل بنود إذن الصرف المباشر -->
      ...
      <table class="w-full text-right border-collapse text-xs">
        <thead>
          <tr class="bg-[#121316] text-gray-300 border-b border-[#3e414c] font-bold select-none">
            ...
            <th class="py-3 px-2 text-center min-w-[130px] w-32">الكمية المنصرفة</th>
            ...
            <th class="py-3 px-3 min-w-[160px]">ملاحظات البند</th>
        ...
        <tbody class="divide-y divide-[#2a2e39]">
          ...
          >
            ...
            <td class="py-2.5 px-2 text-center align-middle">
              <div class="inline-flex items-center justify-center">
                <span
                  ...
                  :class="[
                    (row.current_quantity ?? row.current_stock ?? 0) > 0
                ...
                >
                  {{ formatNumber(row.current_quantity ?? row.current_stock ?? 0) }}
                </span>
              </div>
            </td>
            ...
            <td class="py-2.5 px-3 align-middle">
              <input
                :ref="(el) => setCellRef(el, index, 'notes')"
                type="text"
                v-model="row.notes"
                @keydown="handleNotesKeydown($event, row, index)"
                placeholder="ملاحظات تسليم البند..."
          ...
          <tr v-if="items.length === 0">
            <td colspan="9" class="p-8 text-center text-gray-400 text-xs font-medium">
              لم تسجل أي بند صرف حتى الآن. انقر على الزر أدناه لإضافة البند الأول.
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div class="flex items-center justify-between pt-1">
      ...
      >
        ...
        <span>+ إضافة بند صرف آخر (Ctrl + Enter)</span>
      </button>

      <span class="text-xs text-gray-400 font-mono">
        إجمالي بنود الصرف: <strong class="text-white">{{ items.length }}</strong>
      </span>
    </div>
...
const columnsOrder = ['name', 'unit', 'qty', 'cost', 'notes']
...
const navigateHorizontal = (rowIndex, currentColKey, direction) => {
  ...
  if (targetIdx >= 0 && targetIdx < columnsOrder.length) {
    ...
    focusCell(rowIndex - 1, 'notes', false)
  }
}
...
const handleCostKeydown = (event, row, index) => {
  ...
  if (event.key === 'Enter') {
    event.preventDefault()
    focusCell(index, 'notes', false)
  }
}

const handleNotesKeydown = (event, row, index) => {
  if (checkCommonRowShortcuts(event, index)) return
  ...
  if (event.key === 'ArrowRight') {
    ...
    if (isTextFullySelected(el) || isCursorAtStart(el)) {
      event.preventDefault()
      navigateHorizontal(index, 'notes', -1)
    }
  }
}

<!--src/modules/purchasing/views/issues/components/IssuesFilter.vue-->
<template>
  <div
    class="p-4 bg-[#212634] rounded-2xl border border-[#323a4e] shadow-md text-right space-y-3 font-sans"
    dir="rtl"
  >
    <!-- شبكة الفلاتر التفاعلية بتناسق وخلفيات داكنة موحدة -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-center">
      ...
      <div class="relative flex items-center">
        <input
          type="text"
          v-model="search"
          placeholder="بحث برقم الإذن، رقم الطلب، الملاحظات..."
      ...
      <!-- 2. المستودع الصادر منه -->
      <div class="w-full">
        <WarehouseDropdown id="issues-warehouse-filter" v-model="warehouseFilter" label="" />
      </div>
      ...
      <div class="relative flex items-center">
        ...
        >
          <option value="">كافة حالات الصرف</option>
          <option value="draft">مسودة قيد المراجعة والتجهيز</option>
          <option value="confirmed">تم الصرف والخصم من المخزن</option>
          <option value="cancelled">إذن صرف ملغي</option>
        </select>

...
<template>
  <div class="p-0 bg-transparent space-y-3 font-sans text-right" dir="rtl">
    ...
    <!-- جدول إدخال وتعديل بنود الاستلام المباشر -->
      ...
      <table class="w-full text-right border-collapse text-xs">
        <thead>
          <tr class="bg-[#121316] text-gray-300 border-b border-[#3e414c] font-bold select-none">
            ...
            <th class="py-3 px-2 text-center min-w-[130px] w-32">الكمية المستلمة</th>
            <th class="py-3 px-2 text-center min-w-[130px] w-32">الكمية المقبولة</th>
            <th class="py-3 px-2 text-center min-w-[150px] w-40">المرفوض & السبب</th>
            <th class="py-3 px-2 text-center min-w-[110px] w-28">تكلفة الوحدة</th>
            <th class="py-3 px-2 text-center min-w-[120px] w-32">إجمالي التكلفة</th>
        ...
        <tbody class="divide-y divide-[#2a2e39]">
          ...
          >
            ...
            <td class="py-2.5 px-2 align-middle">
              <div class="flex flex-col items-center justify-center gap-1">
                <div
                  class="flex items-center w-full max-w-[130px] bg-[#16171b] border border-[#3e414c] rounded-lg overflow-hidden focus-within:border-sky-400 focus-within:ring-1 focus-within:ring-sky-400/40"
                >
                  <button
                    type="button"
                    tabindex="-1"
                    @click="decrementQuantity(row, 'quantity_received')"
                  ...
                  <input
                    :ref="(el) => setCellRef(el, index, 'qtyReceived')"
                    type="text"
                    inputmode="decimal"
                    :value="formatNumber(row.quantity_received)"
                    @input="handleQuantityReceivedChange(row, unformatNumber($event.target.value))"
                    @keydown="handleQtyReceivedKeydown($event, row, index)"
                    class="w-full h-9 bg-transparent text-sky-400 text-xs font-mono font-black text-center outline-none"
                    placeholder="1"
                  />
                  ...
                  <button
                    type="button"
                    tabindex="-1"
                    @click="incrementQuantity(row, 'quantity_received')"
                    class="w-8 h-9 bg-[#21242e] hover:bg-[#2c303d] text-gray-300 flex items-center justify-center font-bold text-sm select-none transition-colors"
                  >
                    +
                  </button>
            ...
            <!-- 5. الكمية المقبولة (للمخزن) -->
            <td class="py-2.5 px-2 align-middle">
              <div class="flex flex-col items-center justify-center gap-1">
                <div
                  class="flex items-center w-full max-w-[130px] bg-[#16171b] border border-[#3e414c] rounded-lg overflow-hidden focus-within:border-emerald-400 focus-within:ring-1 focus-within:ring-emerald-400/40"
                >
                  <button
                    type="button"
                    tabindex="-1"
                    @click="decrementQuantity(row, 'quantity_accepted')"
                    class="w-8 h-9 bg-[#21242e] hover:bg-[#2c303d] text-gray-300 flex items-center justify-center font-bold text-sm select-none transition-colors"
                  >
                    -
                  </button>
                  ...
                  <input
                    :ref="(el) => setCellRef(el, index, 'qtyAccepted')"
                    type="text"
                    inputmode="decimal"
                    :value="formatNumber(row.quantity_accepted)"
                    @input="handleQuantityAcceptedChange(row, unformatNumber($event.target.value))"
                    @keydown="handleQtyAcceptedKeydown($event, row, index)"
                    class="w-full h-9 bg-transparent text-emerald-400 text-xs font-mono font-black text-center outline-none"
                    placeholder="1"
                  />
                  ...
                  <button
                    type="button"
                    tabindex="-1"
                    @click="incrementQuantity(row, 'quantity_accepted')"
                    class="w-8 h-9 bg-[#21242e] hover:bg-[#2c303d] text-gray-300 flex items-center justify-center font-bold text-sm select-none transition-colors"
                  >
                    +
                  </button>
            ...
            <!-- 6. الكمية المرفوضة وسبب الرفض -->
            <td class="py-2.5 px-2 align-middle">
              <div class="flex flex-col items-center justify-center gap-1.5">
                <div
                  class="flex items-center w-full max-w-[140px] bg-[#16171b] border border-[#3e414c] rounded-lg overflow-hidden focus-within:border-rose-400 focus-within:ring-1 focus-within:ring-rose-400/40"
                  :class="parseFloat(row.quantity_rejected) > 0 ? 'border-rose-500/60' : ''"
                >
                  <button
                    type="button"
                    tabindex="-1"
                    @click="decrementQuantity(row, 'quantity_rejected')"
                    class="w-8 h-9 bg-[#21242e] hover:bg-[#2c303d] text-gray-300 flex items-center justify-center font-bold text-sm select-none transition-colors"
                  >
                    -
                  </button>
                  ...
                  <input
                    :ref="(el) => setCellRef(el, index, 'qtyRejected')"
                    type="text"
                    inputmode="decimal"
                    :value="formatNumber(row.quantity_rejected)"
                    @input="handleQuantityRejectedChange(row, unformatNumber($event.target.value))"
                    @keydown="handleQtyRejectedKeydown($event, row, index)"
                    class="w-full h-9 bg-transparent text-xs font-mono font-black text-center outline-none"
                    :class="
                      parseFloat(row.quantity_rejected) > 0 ? 'text-rose-400' : 'text-gray-400'
                    "
                    placeholder="0"
                  />
                  ...
                  <button
                    type="button"
                    tabindex="-1"
                    @click="incrementQuantity(row, 'quantity_rejected')"
                    class="w-8 h-9 bg-[#21242e] hover:bg-[#2c303d] text-gray-300 flex items-center justify-center font-bold text-sm select-none transition-colors"
                  >
                    +
                  </button>
                ...
                <!-- إظهار حقل سبب الرفض عند وجود كمية مرفوضة -->
                <input
                  v-if="parseFloat(row.quantity_rejected) > 0"
                  :ref="(el) => setCellRef(el, index, 'rejectReason')"
                  type="text"
                  v-model="row.rejection_reason"
                  @keydown="handleRejectReasonKeydown($event, row, index)"
                  placeholder="سبب الرفض (تالف، كسر...)"
                  class="w-full max-w-[140px] h-8 px-2 text-[10px] bg-rose-950/30 border border-rose-800/50 rounded-lg text-rose-300 placeholder-rose-500/60 outline-none focus:border-rose-500 transition-all text-center"
                />
            ...
            <td class="py-2.5 px-2 align-middle">
              <div class="flex flex-col items-center justify-center gap-1">
                <input
                  ...
                  class="w-full max-w-[100px] h-9 px-2 bg-[#16171b] border border-[#3e414c] hover:border-gray-500 focus:border-[#e05e2b] rounded-lg text-white text-xs font-mono font-bold text-center outline-none focus:ring-1 focus:ring-[#e05e2b]/40 transition-all"
                  placeholder="0.00"
                />
          ...
          <tr v-if="items.length === 0">
            <td colspan="10" class="p-8 text-center text-gray-400 text-xs font-medium">
              لم تسجل أي بند استلام حتى الآن. انقر على الزر أدناه لإضافة البند الأول.
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div class="flex items-center justify-between pt-1">
      ...
      >
        ...
        <span>+ إضافة بند استلام آخر (Ctrl + Enter)</span>
      </button>

      <span class="text-xs text-gray-400 font-mono">
        إجمالي بنود الاستلام: <strong class="text-white">{{ items.length }}</strong>
      </span>
    </div>
...
const columnsOrder = ['name', 'unit', 'qtyReceived', 'qtyAccepted', 'qtyRejected', 'cost']
...
const executeFastSearch = async (queryText, rowIndex, immediate = false) => {
  ...
  const performRequest = async () => {
    ...
    try {
      ...
      // بحث محلي إضافي في حالة عدم العثور عبر السيرفر وكان المتجر يحتوي أصناف
...
const handleSelectProduct = (row, product, index) => {
  ...
  if (isSuccess) {
    focusCell(index, 'qtyReceived', false)
  } else {
...
const handleQtyReceivedKeydown = (event, row, index) => {
  if (checkCommonRowShortcuts(event, index)) return

  if (event.key === '+' || event.key === '=') {
    event.preventDefault()
    props.incrementQuantity(row, 'quantity_received')
    return
  }

  if (event.key === '-') {
    event.preventDefault()
    props.decrementQuantity(row, 'quantity_received')
    return
  }
  ...
  if (event.key === 'ArrowDown') {
    event.preventDefault()
    if (index < props.items.length - 1) {
      focusCell(index + 1, 'qtyReceived', true)
    } else {
      handleAddNewLineWithFocus()
    }
    return
  ...
  if (event.key === 'ArrowLeft') {
    ...
    if (isTextFullySelected(el) || isCursorAtEnd(el)) {
      event.preventDefault()
      navigateHorizontal(index, 'qtyReceived', 1)
    }
    return
  }
  ...
  if (event.key === 'Enter') {
    event.preventDefault()
    focusCell(index, 'qtyAccepted', false)
  }
}

const handleQtyAcceptedKeydown = (event, row, index) => {
  if (checkCommonRowShortcuts(event, index)) return

  if (event.key === '+' || event.key === '=') {
    event.preventDefault()
    props.incrementQuantity(row, 'quantity_accepted')
    return
  }

  if (event.key === '-') {
    event.preventDefault()
    props.decrementQuantity(row, 'quantity_accepted')
    return
  }
  ...
  if (event.key === 'ArrowDown') {
    event.preventDefault()
    if (index < props.items.length - 1) {
      focusCell(index + 1, 'qtyAccepted', true)
    } else {
      handleAddNewLineWithFocus()
    }
    return
  ...
  if (event.key === 'ArrowLeft') {
    ...
    if (isTextFullySelected(el) || isCursorAtEnd(el)) {
      event.preventDefault()
      navigateHorizontal(index, 'qtyAccepted', 1)
    }
    return
  }
  ...
  if (event.key === 'Enter') {
    event.preventDefault()
    focusCell(index, 'qtyRejected', false)
  }
}

const handleQtyRejectedKeydown = (event, row, index) => {
  if (checkCommonRowShortcuts(event, index)) return

  if (event.key === '+' || event.key === '=') {
    event.preventDefault()
    props.incrementQuantity(row, 'quantity_rejected')
    return
  }

  if (event.key === '-') {
    event.preventDefault()
    props.decrementQuantity(row, 'quantity_rejected')
    return
  }
  ...
  if (event.key === 'ArrowDown') {
    event.preventDefault()
    if (index < props.items.length - 1) {
      focusCell(index + 1, 'qtyRejected', true)
    } else {
      handleAddNewLineWithFocus()
    }
    return
  ...
  if (event.key === 'ArrowLeft') {
    ...
    if (isTextFullySelected(el) || isCursorAtEnd(el)) {
      event.preventDefault()
      if (parseFloat(row.quantity_rejected) > 0) {
        focusCell(index, 'rejectReason', false)
      } else {
        navigateHorizontal(index, 'qtyRejected', 1)
      }
    }
    return
  }
...
const handleRejectReasonKeydown = (event, row, index) => {
  if (checkCommonRowShortcuts(event, index)) return

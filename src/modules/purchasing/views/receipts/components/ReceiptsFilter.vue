<!--src/modules/purchasing/views/receipts/components/ReceiptsFilter.vue-->
<template>
  <div
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-center">
      ...
      <div class="relative flex items-center">
        <input
          type="text"
          v-model="search"
          placeholder="بحث برقم السند، بوليصة الشحن، إذن التسليم، الملاحظات..."
      ...
      <!-- 2. المستودع المستلم -->
      <div class="w-full">
        <WarehouseDropdown id="receipts-warehouse-filter" v-model="warehouseFilter" label="" />
      </div>

      <!-- 3. حالة سند الاستلام المخزني -->
      <div class="relative flex items-center">
        <select
        ...
        >
          <option value="">كافة حالات الاستلام</option>
          <option value="draft">مسودة قيد المراجعة والفحص</option>
          <option value="received">تم الاستلام وترحيل المخزون</option>
          <option value="cancelled">سند استلام ملغي</option>
        </select>
